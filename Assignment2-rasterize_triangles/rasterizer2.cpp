...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    max_x = std::max(t.v[2].x(),std::max(t.v[0].x(),t.v[1].x()));
    max_y = std::max(t.v[2].y(),std::max(t.v[0].y(),t.v[1].y()));
    min_x = std::min(t.v[2].x(),std::min(t.v[0].x(),t.v[1].x()));
    min_y = std::min(t.v[2].y(),std::min(t.v[0].y(),t.v[1].y()));
    ...
    for(int x = min_x;x <= max_x;++x)
        ...
        {
            bool is_set = insideTriangle((float)x+0.5,(float)y+0.5,t.v);
